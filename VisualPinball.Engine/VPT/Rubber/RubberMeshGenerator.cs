...
namespace VisualPinball.Engine.VPT.Rubber
{
	public class RubberMeshGenerator
	{
		...
		private Mesh GetMeshNew(float playfieldHeight, float meshHeight, int detailLevel, int acc = -1, bool createHitShape = false, float margin = 0f)
		{
			var mesh = new Mesh();
		...
		private Mesh GetMesh(float playfieldHeight, float meshHeight, int detailLevel, int acc = -1, bool createHitShape = false, float margin = 0f)
		{
			var newMesh = true;
			if (newMesh)
				return GetMeshNew(playfieldHeight, meshHeight, detailLevel, acc, createHitShape, margin);
			else
				return GetMeshOld(playfieldHeight, meshHeight, detailLevel, acc, createHitShape, margin);
		}


		private Mesh GetMeshOld(float playfieldHeight, float meshHeight, int detailLevel, int acc = -1, bool createHitShape = false, float margin = 0f)
		{
			var mesh = new Mesh();
			var accuracy = (int)(10.0f * 1.2f);
