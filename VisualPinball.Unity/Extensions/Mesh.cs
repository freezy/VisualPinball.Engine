...
namespace VisualPinball.Unity.Extensions
{
	public static class Mesh
	{
		...
		public static UnityEngine.Mesh ToUnityMesh(this Engine.VPT.Mesh vpMesh, RenderObject ro, string name = null)
		{
			var mesh = new UnityEngine.Mesh { name = name ?? vpMesh.Name };

			Vector3 ea = ro.TransformationMatrix.ToUnityMatrix().rotation.eulerAngles*-1f;
			Matrix4x4 m = new Matrix4x4();
			m.SetTRS(Vector3.zero, Quaternion.Euler(-90,0,0), Vector3.one);
			...
			for (var i = 0; i < vertices.Length; i++) {
				...
				vertices[i] = GlobalMatrix.MultiplyPoint(vertex.ToUnityVector3());
