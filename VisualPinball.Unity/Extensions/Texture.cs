	...
	public static class Texture
	{
		public static Texture2D ToUnityTexture(this Engine.VPT.Texture vpTex)
		{
			...
			if (vpTex.Data.Bitmap != null) {
				...
				unityTex = new Texture2D(bmp.Width, bmp.Height, TextureFormat.RGB24,true);
				unityTex.name = vpTex.Name;
				unityTex.SetPixels32(colArr);
				unityTex.Apply();

			} else {
				unityTex = new Texture2D(vpTex.Width, vpTex.Height, TextureFormat.RGB24, true);
		...
		public static Texture2D ToUnityHDRTexture(this Engine.VPT.Texture vpTex) {
			...
			if (vpTex.Data.Bitmap != null) {
				...
				unityTex = new Texture2D(bmp.Width, bmp.Height, TextureFormat.RGBAFloat, false);
				unityTex.name = vpTex.Name;			
				unityTex.SetPixels32(colArr);
				unityTex.Apply();

			} else {
				unityTex = new Texture2D(vpTex.Width, vpTex.Height, TextureFormat.RGBAFloat, false);
				unityTex.LoadImage(vpTex.FileContent);
			}
			return unityTex;
		}
