...
Click **Add V Brace** to add an open support that runs down the left side of the rail envelope, around a rounded bottom, and up the right side. **Position** places it along the route, **Offset X/Z** translates it in the route-local cross section, and **Ring Density** controls the minimum sampling density of its rounded corner or corners; the generator always keeps corner steps at or below 15° to preserve the visible wire thickness. On the default four-wire layout, the default 53.13° **Angle** and 85-unit arm lengths place the straight arms tangent to the outside surfaces of both bottom and middle rails. **Left Length** and **Right Length** independently control how far each arm extends from the theoretical V tip, **Rotation** rotates the complete support around the spline tangent, and **Corner Radius** controls its requested centerline bend radius. The bend is sampled as a circular fillet and the wire keeps the component-wide diameter through the corner.

Enable the V brace's **Straight Line** option to replace the single rounded tip with a flat bottom and two rounded corners. **Height** is measured upward from the theoretical V tip to the horizontal line where that bottom crosses the arms, so increasing it widens and raises the flat. It is capped at 90% of the shorter arm's vertical reach while the option is enabled; editing arm length or angle while it is disabled leaves the stored height unchanged. A zero-height straight section behaves like the ordinary rounded V. Corner radii are reduced only as needed to fit the available adjacent spans; configurations that cannot fit at least one wire radius are omitted instead of producing a self-intersecting tube. Increasing the global wire diameter raises too-small corner radii to half that diameter, and lowering the diameter later does not reduce an authored or previously raised radius. With fewer than four active rails, the V brace remains available and uses the active rail envelope as its automatic origin; use its offsets, angle, lengths, and rotation to place it precisely.
