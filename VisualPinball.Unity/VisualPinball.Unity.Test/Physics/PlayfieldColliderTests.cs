...
namespace VisualPinball.Unity.Test
{
	public class PlayfieldColliderTests
	{
		...
		public void OnlyAutoGeneratedPlayfieldMeshesAreProcedural(bool autoGenerate, bool expected)
		{
			var gameObject = new GameObject("Playfield");

			try {
				gameObject.AddComponent<PlayfieldComponent>();
				var meshComponent = gameObject.AddComponent<PlayfieldMeshComponent>();
				meshComponent.AutoGenerate = autoGenerate;

				var property = typeof(PlayfieldMeshComponent).GetProperty("IsProcedural", BindingFlags.Instance | BindingFlags.NonPublic);
				Assert.That(property, Is.Not.Null);
				Assert.That(property.GetValue(meshComponent), Is.EqualTo(expected));
			} finally {
				Object.DestroyImmediate(gameObject);
			}
		}
